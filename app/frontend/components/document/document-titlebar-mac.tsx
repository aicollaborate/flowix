...
import { LinkSimpleIcon, CopyIcon, PushPinIcon, PushPinSlashIcon, ExportIcon, FileDocIcon, MicrosoftWordLogoIcon, ClockIcon, TrashIcon } from '@phosphor-icons/react';
      ...
      <div className="ml-auto flex items-center gap-3 pr-2">
        ...
        {currentMemo && (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-[200px] px-1 py-2 space-y-2">
              <DropdownMenuItem
                onClick={onCopyLink}
                className="flex items-center cursor-pointer rounded-xl px-2 hover:bg-[var(--muted)]"
              >
              ...
              <div className="h-px bg-black/10 my-[1px] mx-1" />
              <DropdownMenuItem
                onClick={onExportMarkdown}
                className="flex items-center cursor-pointer rounded-xl px-2 hover:bg-[var(--muted)]"
              >
                <ExportIcon className="w-4 h-4 mr-2" /> 导出为 Markdown
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={onExportPdf}
                className="flex items-center cursor-pointer rounded-xl px-2 hover:bg-[var(--muted)]"
              >
                <FileDocIcon className="w-4 h-4 mr-2" /> 导出为 PDF
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={onExportWord}
                className="flex items-center cursor-pointer rounded-xl px-2 hover:bg-[var(--muted)]"
              >
                <MicrosoftWordLogoIcon className="w-4 h-4 mr-2" /> 导出为 Word
              </DropdownMenuItem>
              <div className="h-px bg-black/10 my-[1px] mx-1" />
              <DropdownMenuItem className="flex items-center cursor-pointer rounded-xl px-2 hover:bg-[var(--muted)]">
                <ClockIcon className="w-4 h-4 mr-2" /> 历史版本
              </DropdownMenuItem>
              <DropdownMenuItem className="flex items-center cursor-pointer rounded-xl px-2 hover:bg-[var(--muted)] text-red-500">
