...
export function AlertDialogCancel({ children, onClick, className }: AlertDialogCancelProps) {
	...
	return (
		<button
			...
			className={cn(
				'inline-flex items-center justify-center rounded-lg text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 h-8 px-3 py-1 border border-[var(--border)] bg-[var(--secondary)] hover:bg-[var(--accent)] hover:text-[var(--secondary-foreground)]',
				className
			)}
		>
			{children}
