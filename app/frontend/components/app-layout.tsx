...
import { DocumentTitlebarWin } from './document/document-titlebar-win';
import { DocumentTitlebarMac } from './document/document-titlebar-mac';
import { MemoList } from './memo/memo-list';
import { MemoListTitlebarWin } from './memo/memo-list-titlebar-win';
import { MemoListTitlebarMac } from './memo/memo-list-titlebar-mac';
import { Infinity, ChevronsUpDown, ListTodo } from 'lucide-react';
import { AgentChatRoot } from './agent/agent-root';
import { useMemoStore, useDocumentStore, useSettingsStore } from '../lib/store';
import { files, memos as memosClient, dialogs, type SaveFileFilter, windows } from '../lib/tauri/client';
import { WindowsTitlebarControls } from './windows-titlebar-controls';
...
export function AppLayout() {
  const { memos, selectedMemo, selectedNotebook, refreshTrigger, activeSort, setActiveFilter, loadMemos, setSelectedMemo, updateMemoMeta } = useMemoStore();
  ...
  const [memoColWidth, setMemoColWidth] = useState(350);
  ...
  const handleCopyFullText = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[AppLayout] Failed to copy document content:', error);
      toast.error('复制失败');
    }
  ...
  const handleCopyLink = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[AppLayout] Failed to copy document link:', error);
      toast.error('复制失败');
    }
  ...
  const handleTogglePin = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[AppLayout] Failed to toggle pin:', error);
      toast.error(wasFavorited ? '取消置顶失败' : '置顶失败');
    }
  ...
  const getExportableDocument = useCallback(async (): Promise<ExportableDocument | null> => {
    ...
    if (!raw) {
      try {
        ...
        console.warn('[AppLayout] Failed to read document for export:', error);
        toast.error('读取文档失败');
        return null;
      }
    }
  ...
  const handleExportWord = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[AppLayout] Failed to convert markdown for Word export:', error);
      toast.error('导出失败');
      return;
    }
  ...
  const handleExportPdf = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[AppLayout] Failed to convert markdown for PDF export:', error);
      toast.error('导出失败');
      return;
    }

    const target = await promptExportTarget(doc, 'pdf', { name: 'PDF', extensions: ['pdf'] });
    if (!target) return;

    let pdfBase64: string;
    try {
      pdfBase64 = await renderHtmlToPdfBase64(doc.title, bodyHtml);
    } catch (error) {
      console.warn('[AppLayout] Failed to render PDF:', error);
      toast.error('PDF 生成失败');
      return;
    }

    const ok = await dialogs.writeExportBinaryFile(target, pdfBase64);
    toast[ok ? 'success' : 'error'](ok ? '已导出 PDF' : '导出失败');
  }, [promptExportTarget, requireExportableDocument]);
  ...
  useEffect(() => {
    ...
    async function loadNotebookTodoCount() {
      ...
      try {
        ...
        if (!cancelled) {
          console.warn('[AppLayout] Failed to read memo metadata todos:', error);
          setTodoCount(0);
        }
      }
    }
  ...
  useEffect(() => {
    ...
    memosClient.getLaunchOpenFiles()
      ...
      .catch((error) => console.warn('[AppLayout] Failed to read launch files:', error));
  ...
  useEffect(() => {
    ...
    getCurrentWindow().onDragDropEvent((event) => {
      ...
      console.warn('[AppLayout] Failed to listen for file drops:', error);
    });
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          ...
          <div className="h-6 shrink-0 flex items-center text-xs text-gray-500 border-t border-black/5 bg-white">
            <div className="h-full flex items-center gap-2">
              <button className="h-full flex px-1 bg-[#4E62E5] items-center hover:bg-[#6B7CF0]">
                  <span className="pl-2 pr-1 h-full py-0 min-w-[30px] max-w-[80px] text-white flex items-center gap-1 overflow-hidden whitespace-nowrap">{selectedNotebook?.name || '未选择笔记本'}</span>
              ...
              >
                ...
                <span>{todoCount}</span>
                            ...
                            <span className="text-gray-400">{charCount}字</span>
            </div>
            <div className="flex-1" />
            <button onClick={toggleAgentPanelVisible} className="h-full flex items-center gap-1 px-1.5 py-0 hover:bg-black/5 mr-2">
              <Infinity className="w-3.5 h-3.5" />
              <span>Claude Code 已连接</span>
            </button>
          </div>
        </div>
      </div>
