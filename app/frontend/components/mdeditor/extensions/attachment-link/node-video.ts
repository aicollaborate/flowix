...
class VideoView implements ProseMirrorNodeView {
    ...
    stopEvent(event: Event): boolean {
        ...
        return !!target.closest('.tiptap-video-attachment');
    }

    ignoreMutation(mutation: ViewMutationRecord): boolean {
        const target = mutation.target as HTMLElement;
        const isVideoContainer = target.closest('.tiptap-video-attachment');
    ...
    renderHTML({ HTMLAttributes }) {
        ...
        return [
            ...
            ['video', mergeAttributes(
                { controls: 'true', style: 'max-height: 290px' },
