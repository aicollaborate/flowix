...
function escapeBackslashesOutsideInlineCode(line: string): string {
...
function preservePlainTextBackslashes(markdown: string): string {
  ...
  return lines.map((line, index) => {
    ...
    return escapeBackslashesOutsideInlineCode(line);
  }).join('\n');
}

function normalizeMarkdownOutput(markdown: string): string {
  return preservePlainTextBackslashes(markdown)
    .replace(/^(---\r?\n[\s\S]*?\r?\n---)\r?\n{2,}/, '$1\n');
}
...
const MarkdownEscape = Extension.create({
  ...
  parseMarkdown(token, h) {
    return h.createTextNode(token.text || '');
  },
});
