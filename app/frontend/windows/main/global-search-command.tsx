    ...
    <>
      ...
      <CommandGroup heading="导航">
        <CommandItem value="filter-favorited" onSelect={() => handleFilter('favorited')}>
          <Star />
          <span>置顶</span>
          {/* 右侧组: Check + CommandShortcut 用 ml-auto 整体推到右; override CommandShortcut
              自身 ml-auto 让两者紧贴, 否则会分摊剩余空间导致 Check 浮在中间 */}
          <span className="ml-auto flex items-center gap-2">
            {activeFilter === 'favorited' && <Check className="text-[var(--primary)]" />}
            <CommandShortcut className="!ml-0">⇧⌘F</CommandShortcut>
          </span>
        </CommandItem>
        ...
        <CommandItem value="filter-todos" onSelect={() => handleFilter('todos')}>
          ...
          <span className="ml-auto flex items-center gap-2">
            ...
            <CommandShortcut className="!ml-0">⌘T</CommandShortcut>
          </span>
        </CommandItem>
      </CommandGroup>
          ...
          notebooks.map((nb) => {
            ...
            return (
              ...
              >
                ...
                {nb.isDefault && (
                  <span className="rounded px-1.5 py-0.5 text-[10px] bg-[var(--accent)] text-[var(--primary)]">
                    默认
                  </span>
                )}
