 ...
 *
 *   导航
 *     palette.search  ⌘K          no-input — 打开命令面板
 *     menu.open       ⌘,          no-input — 打开偏好设置窗口
 *
 *   Memo
 ...
 *     panel.memoList.toggle Shift+M    no-input — 显示/隐藏 memo 列表 (左栏)
 *     panel.agent.toggle   Shift+A    no-input — 显示/隐藏 Agent 面板 (右栏)
 *
 *   系统
 ...
 * Tiptap 编辑器 — 打开搜索替换面板。
 *
 * scope: 'window' — 与原 comn-tiptap-editor.tsx:307-313 行为一致, 任何地方
 * 按 ⌘F 都能打开面板 (无编辑器挂载时, run 返回 false, 落到浏览器默认)。
 *
 * handler 由 ComnTiptapEditor 组件 mount 时 push, 内部读 onSearchPanelOpenChangeRef。
 */
 ...
 *
 * scope: 'editor' — 与 editor.undo/redo 同款, 仅当编辑器获得焦点 (comn-tiptap-editor
 * 调 useShortcutScope('editor')) 时生效。 handler 通过 pushHandler 间接引用
 * 当前 mounted 的 editor 实例 (在 comn-tiptap-editor.tsx 里 push), 卸载时 pop。
 *
...
/**
 * 打开全局搜索 / 命令面板 (GlobalSearchCommand)。
 *
 * 实现: dispatch `flowix:open-palette` 事件, memo-list.tsx 监听后
 * `setSearchCommandOpen(true)`。沿用仓库里 `flowix:open-create-notebook` /
 * `flowix:request-delete-memo` 的 CustomEvent 解耦模式 — 命令面板的
 * 状态留在 memo-list 内部, 任何位置都能触发。
 *
 * scope: 'no-input' — 焦点在 input/textarea 时不触发 (用户想输入 "k")。
 * 当命令面板已经打开且焦点在其 input 上时, `editable=true` 同样使此 action
 * 被 scope 过滤, 不会和面板内部的 cmdk 导航键冲突。
 */
export const paletteSearchAction = defineAction({
  id: 'palette.search',
  title: '打开命令面板',
  description: '打开全局搜索 / 命令面板, 输入即搜索, 顶部列表为常用命令。',
  group: '导航',
  scope: 'no-input',
  defaultBinding: {
    mac: 'Mod+K',
  ...
  run: () => {
    window.dispatchEvent(new CustomEvent('flowix:open-palette'));
  },
});
 ...
 *
 * scope: 'no-input' — 与其它单字母 chord (memo.create / palette.search / menu.open)
 * 一致, 焦点在 input/textarea/contenteditable 时不触发, 防止用户敲大写 M
 * (e.g. "MemoList" 标题) 时误触。
 *
...
export const memoListToggleAction = defineAction({
  ...
  title: '显示/隐藏 memo 列表',
  description: '切换左栏 memo 列表的显示与隐藏 (Shift+M)。',
  group: '视图',
  scope: 'no-input',
  defaultBinding: {
    mac: 'Shift+M',
 ...
 *
 * scope: 'no-input' — 同 memoListToggleAction, 单字母 chord 必须避开 input。
...
export const agentPanelToggleAction = defineAction({
  ...
  description: '切换右栏 Agent 面板的显示与隐藏 (Shift+A)。',
  group: '视图',
  scope: 'no-input',
  defaultBinding: {
    mac: 'Shift+A',
