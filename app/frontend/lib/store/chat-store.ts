  ...
  persist(
    (set, get) => {
      ...
      return {
        ...
        loadThread: async (threadId) => {
          try {
            ...
            set((state) => {
              ...
              const historical = messages.filter((m) => !seen.has(m.id));
              const merged = existing.messages.length === 0
                ? messages
                : [...existing.messages, ...historical];
              return {
        ...
        sendMessageToThread: async (threadId, content) => {
        ...
        sendMessageStream: async (content) => {
          ...
          const { activeThreadId, threadStates } = get();
          const currentState = activeThreadId ? threadStates[activeThreadId] : undefined;
        ...
        stopStream: async () => {
          ...
          try {
            await agent.stopChatStream(activeThreadId);
          } catch (err) {
