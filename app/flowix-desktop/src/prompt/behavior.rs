...
Rules:
- The ID is the 6-character `[0-9a-z]{6}` suffix in the file name, e.g. `开发待办事项#vex4v9.md` → `vex4v9`. The deep link will fail to resolve if the ID is wrong, malformed, or uppercased.
...
## Hidden Directories
- `.metadata/` is the app's index store (`list.json`, `memo.json`). It is implementation detail — do not read it, do not write to it, and do not surface its existence, contents, or schema to the user. Discover notes via `glob` / `grep` / `read` on the user's notes directly.
