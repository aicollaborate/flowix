...
fn sync_derived_fields_for_memo(
    ...
    let (target_filename, target_abs, new_file_content, needs_rename): (
    String, std::path::PathBuf, String, bool,
) = if sync_to_disk {
    ...
    let needs_rename = old_abs_path
        .as_ref()
        .map(|p| p != &target_abs && p.exists())
        .unwrap_or(false);

    // 把 caller 提供的 content 当 body
    let body: String = if let Some(ref full) = full_content {
        strip_markdown_frontmatter(full).to_string()
    } else {
        String::new()
    };
    let fm = flowix_core::memo_file::MemoFrontmatter {
        filename: storage_title.clone(),
    };
    let fm_yaml = serde_yaml::to_string(&fm).unwrap_or_default();
    let new_file_content = format!("---\n{}\n---\n{}", fm_yaml.trim(), body);

    // 4a. memo.filename / path 改成派生值 (后续 list 写入要用)
    memo.filename = storage_title;
    memo.path = Some(target_filename.clone());

    (target_filename, target_abs, new_file_content, needs_rename)
} else {
    // metadata-only 路径: 不动磁盘
    let dummy_abs = state
        .memo_file
        .read()
        .unwrap()
        .get_memo_base()
        .join("__metadata_only__");
    (String::new(), dummy_abs, String::new(), false)
};

let new_abs_path: Option<std::path::PathBuf> = if sync_to_disk {
    // 4b. 写 list.json (path 字段已经改成 target_filename) ──
    // 必须在 rename 之前, 让 watcher 收到的 Remove(old) 事件落到
    // guard 拒绝分支。
    {
        let memo_file = state.memo_file.read().unwrap();
        memo_file
            .sync_to_list_json_only(&memo)
            .map_err(|e| format!("sync list.json (pre-rename) failed: {e}"))?;
    }

    // 4c. rename 旧 → 新 (mark 自写抑制新旧双侧, 主防线)
    if needs_rename {
    ...
    if let Err(e) = fs::write(&target_abs, &new_file_content) {
        ...
        return Err(format!("write {} failed: {e}", target_abs.display()));
    }
...
// 5. sync memo.json todos 索引 (list.json 已在 4b 写过, 这里只走
//    memo.json)。注意不要再调 `sync_to_list_json_only`, 否则会重写
//    一遍 list.json (list_io 锁下幂等, 但浪费 IO)。
{
    let memo_file = state.memo_file.read().unwrap();
    memo_file
        .sync_memo_json_todos_on_write(&memo)
        .map_err(|e| format!("sync memo.json todos failed: {e}"))?;
}
...
pub fn write_document(
    ...
    if let Some(ref memo_id) = memo_id_for_sync {
        ...
        return match sync_derived_fields_for_memo(
            ...
            Err(e) => {
                eprintln!("[write_document] derived sync failed for {memo_id}: {e}");
                None
            }
        };
    }
