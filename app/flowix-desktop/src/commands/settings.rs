//! 偏好 / AI 配置 IPC — `~/.flowix/preference.json` + `~/.flowix/ai_config.json`。
//!
...
pub fn set_preference(
    ...
    state
        ...
        .map(|_| {
            app.emit(USER_CONFIG_CHANGED_EVENT, "preference")
                .map_err(|e| e.to_string())
        })
        .map_err(|e| e.to_string())?
}

/// AI 模型配置 (ai_config.json) — 走 ~/.flowix/ai_config.json
#[tauri::command]
...
pub fn set_ai_config(
    ...
    state
        ...
        .map(|_| {
            app.emit(USER_CONFIG_CHANGED_EVENT, "ai_config")
                .map_err(|e| e.to_string())
        })
        .map_err(|e| e.to_string())?
}
...
pub fn update_watcher_config(
    ...
    state
        ...
        .set_preference(pref)
        .map(|_| {
            app.emit(USER_CONFIG_CHANGED_EVENT, "watcher")
                .map_err(|e| e.to_string())
        })
        .map_err(|e| e.to_string())?
}
