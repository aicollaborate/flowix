...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .manage(memo_watcher.clone())
            ...
            let dispatcher: crate::watcher::dispatcher::SharedDispatcher =
                std::sync::Arc::new(crate::watcher::dispatcher::TauriDispatcher::new(
                    app.handle().clone(),
                ));
            ...
            {
                let watcher_cfg = user_config_for_watcher
                    .get_preference()
                    .watcher
                    .clone();
                memo_watcher.write().unwrap().set_whitelist(watcher_cfg);
                ...
                app.listen("user-config-changed", move |event| {
                    ...
                    if kind == "preference" || kind == "watcher" {
                        ...
                        w_for_evt.write().unwrap().set_whitelist(new_cfg);
