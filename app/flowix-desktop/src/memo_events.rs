...
/// PR3: 通过 dispatcher 派发 — 走 `crate::watcher::dispatcher::EventDispatcher`
/// 抽象。`emit()` 是直接调 `app.emit` 的薄壳; `emit_via_dispatcher` 让未来
/// 多 channel 扩展 (attachment-event / tag-event) 共用同一份 trait 接口。
///
/// 注: id 兜底表由 `MemoWatcher` 自己维护, 不属于 dispatcher 职责。
/// 当前 fs_watcher / agent.rs 都走 `emit(app, ...)` 路径, 不会进这里。
...
mod tests {
    ...
    fn sample_memo() -> Memo {
        Memo {
            ...
            icon: None,
            colors: vec![],
            path: Some("Sample#abc12.md".to_string()),
        }
    }
