...
//! 误处理。
//!
//! 配置加载: 当前 PR1 默认值直接 `Default::default()`, 后续 PR 接入
//! `preference.json::watcher` 字段, 运行时 `Arc<RwLock<WhitelistConfig>>`
//! 热更新。
//!
...
impl WhitelistConfig {
    /// 加载或返回默认。后续 PR 接入 `preference.json::watcher` 字段。
...
mod tests {
    ...
    fn default_skips_metadata_dir() {
        let w = WhitelistConfig::default();
        assert_eq!(
            w.allows(Path::new("/x/.metadata/list.json")),
