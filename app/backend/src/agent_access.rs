...
/// agent_access.json 的文件名。 与 `preference.json` / `ai_config.json`
...
/// Agent 访问目录的 add_folder / replace_config 错误。`UserConfigError`
/// (`Io` / `Json`) 已经覆盖底层写盘, 业务级只多一个 `PathConflict`
/// (要加的 folder 路径已存在)。
...
impl AgentAccessStore {
    ...
    /// notebook 改名时调 (path 不变)。 返回 true = 名字真变了。
    pub fn rename_notebook(&self, notebook_id: &str, new_name: &str) -> bool {
        let mut guard = self.inner.write().unwrap_or_else(|p| p.into_inner());
        let mut changed = false;
        if let Some(entry) = guard
            .entries
            .iter_mut()
            .find(|e| e.kind == AgentAccessKind::Notebook && e.id == notebook_id)
        {
    ...
    /// 切换 enabled。 任意条目都接受 (默认笔记本不再特殊处理 ── 与
    /// 用户确认的产品决策一致)。 返回 true = 真改了。
    ...
    /// 加自定义 folder。 若 path 已经被任一条 entry (notebook 或 folder) 跟踪
    /// 则返回 `PathConflict` ── 路径唯一, 不允许重复条目; 前端据此弹 toast
    /// 并 no-op。 name 缺省时取 path 末段。
    pub fn add_folder(
        &self,
        path: &Path,
        name: Option<&str>,
    ) -> Result<AgentAccessEntry, AgentAccessError> {
        let canonical = canonicalize_for_compare(path);
        let now = chrono::Utc::now().timestamp_millis();
        let derived_name: String = name.map(|n| n.to_string()).unwrap_or_else(|| {
            canonical
                .file_name()
        ...
        if let Some(existing) = guard
            .entries
            .iter()
            .find(|e| path_eq_for_compare(&e.path, &canonical))
        {
            return Err(AgentAccessError::PathConflict(existing.id.clone()));
        }

        let new_entry = AgentAccessEntry {
            id: format!("fld_{}", random_folder_suffix()),
            ...
            added_at: now,
            updated_at: now,
            missing: !canonical.exists(),
        };
        guard.entries.push(new_entry.clone());
        let _ = self.persist_locked(&guard);
        Ok(new_entry)
    }

    /// 删自定义 folder。 kind != Folder 一律 no-op。 返回 true = 真删了。
    pub fn remove_folder(&self, id: &str) -> bool {
        let mut guard = self.inner.write().unwrap_or_else(|p| p.into_inner());
        let before = guard.entries.len();
        guard
            .entries
            .retain(|e| !(e.id == id && e.kind == AgentAccessKind::Folder));
        let removed = guard.entries.len() != before;
        if removed {
            let _ = self.persist_locked(&guard);
        }
...
/// [`crate::memo_file::MEMO_ID_ALPHABET`]。 36^6 ≈ 21.7 亿种,
/// 用户通常 < 20 个 folder, 实际不撞。
#[allow(dead_code)]
fn random_folder_suffix() -> String {
    const ALPHABET: &[u8] = b"0123456789abcdefghijklmnopqrstuvwxyz";
    let nanos = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .map(|d| d.as_nanos())
        .unwrap_or(0);
    // 用 nanos + process id 做 seed, 简单够用, 不需要密码学强度
    let mut state: u128 = nanos.wrapping_mul(0x9E37_79B9_7F4A_7C15);
...
mod tests {
    ...
    #[test]
    fn remove_notebook_does_not_touch_folder() {
        let dir = tempdir();
        let memo_path = dir.join("notebook.json");
        std::fs::write(&memo_path, "[]").unwrap();
        let memo = MemoFile::new(dir.clone(), memo_path);
        ...
        // 注入一个 folder
        let folder = std::env::temp_dir().join("agent_access_test_folder");
        std::fs::create_dir_all(&folder).unwrap();
        let added = store.add_folder(&folder, None).unwrap();
        assert_eq!(added.kind, AgentAccessKind::Folder);

        // remove 一个不存在的 notebook id — 应当返回 false, 且 folder 仍在
        assert!(!store.remove_notebook("nb_ghost"));
        assert_eq!(store.get_config().entries.len(), 1);
    }

    #[test]
    fn add_folder_rejects_path_colliding_with_existing() {
        let dir = tempdir();
        let memo_path = dir.join("notebook.json");
        let path_str = dir.to_string_lossy().to_string();
        let configs = vec![nb("nb_1", "MyNB", &path_str)];
        std::fs::write(&memo_path, serde_json::to_string(&configs).unwrap()).unwrap();
        let memo = MemoFile::new(dir.clone(), memo_path);
        let store = AgentAccessStore::new(dir.clone(), &memo);

        // 试图把同一个路径作为 folder 加进来 — 应 PathConflict
        let p = PathBuf::from(&path_str);
        match store.add_folder(&p, None) {
            Err(AgentAccessError::PathConflict(_)) => {}
            other => panic!("expected PathConflict, got {:?}", other),
        }
    }

    #[test]
    fn set_enabled_is_effective_for_every_entry_including_default() {
        let dir = tempdir();
        let memo_path = dir.join("notebook.json");
        let path_str = dir.to_string_lossy().to_string();
        let mut cfg_nb = nb("nb_default", "Default", &path_str);
        cfg_nb.is_default = true;
        std::fs::write(&memo_path, serde_json::to_string(&vec![cfg_nb]).unwrap()).unwrap();
        let memo = MemoFile::new(dir.clone(), memo_path);
        let store = AgentAccessStore::new(dir.clone(), &memo);

        assert!(store.set_enabled("nb_default", false));
        let cfg = store.get_config();
        assert!(!cfg.entries[0].enabled);
    }

    #[test]
    fn missing_flag_reflects_disk_state() {
        let dir = tempdir();
        let memo_path = dir.join("notebook.json");
        std::fs::write(&memo_path, "[]").unwrap();
        let memo = MemoFile::new(dir.clone(), memo_path);
        ...
        // folder 指向一个真实存在的 tmp dir
        let folder = std::env::temp_dir().join("agent_access_missing_test");
        std::fs::create_dir_all(&folder).unwrap();
        let added = store.add_folder(&folder, None).unwrap();
        assert!(!store.get_config().entries[0].missing);

        // 删了它
        std::fs::remove_dir_all(&folder).unwrap();
        let cfg = store.get_config();
        assert_eq!(cfg.entries[0].id, added.id);
        assert!(cfg.entries[0].missing);
    }
    ...
    fn tempdir() -> PathBuf {
        let base = std::env::temp_dir().join(format!("agent_access_test_{}", std::process::id()));
        let dir = base.join(format!("{}", random_folder_suffix()));
