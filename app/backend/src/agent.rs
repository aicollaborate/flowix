...
impl AgentManager {
    /// 构造时必须传入 4 个共享依赖 ── 与 `AppState` 持有同一份 Arc 引用。
    ...
    /// 测试用 fixture ── 用空 / 临时路径构造 4 个依赖, 不真正读写磁盘。
    ...
    async fn execute_tool_for_thread(
        ...
        if matches!(in_notebook, Some(true)) {
            if let (Some(pk), Some(app)) = (path_key.as_ref(), app_handle) {
                if let Some(watcher) = app
                    .try_state::<std::sync::Arc<std::sync::RwLock<crate::fs_watcher::MemoWatcher>>>(
                    )
                {
                        ...
                        if in_notebook.unwrap_or(false) {
                            ...
                            let memo_after = if !path_existed_before {
                                ...
                                self.memo_file
                                    ...
                                    .ok()
                            } else if let Some(id) = MemoFile::extract_memo_id_from_abs_path(path) {
                            ...
                            if let (Some(app), Some(memo)) = (app_handle, memo_after) {
                                ...
                                if !path_existed_before {
                                    memo_events::emit(app, MemoEvent::Created { memo, source });
                                } else {
                                    memo_events::emit(
                                        app,
                                        MemoEvent::Updated {
                            ...
                            let id =
                                MemoFile::extract_memo_id_from_abs_path(path).unwrap_or_default();
                            let source = if tool_name == "write" {
                                MemoChangeSource::AgentWrite
                            } else {
                                MemoChangeSource::AgentEdit
