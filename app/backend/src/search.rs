...
use crate::memo_file::MemoListEntry;
...
pub struct MemoIndex {
    ...
    entries: HashMap<String, MemoIndexEntry>,
...
impl MemoIndex {
    ...
    /// 读失败或条目不存在的项会被静默跳过 (在 list.json 里的 metadata 仍会进索引,
    /// 只是 body_lower 为空, 搜不到正文).
    pub fn rebuild(&mut self, notebook_id: String, items: Vec<(MemoListEntry, String)>) {
        self.mark_unloaded();
    ...
    pub fn upsert(&mut self, entry: MemoListEntry, full_md: &str) {
        if !self.loaded {
            return;
        }
    ...
    fn insert_entry(&mut self, entry: MemoListEntry, full_md: &str) {
        let id = entry.id.clone();
        let title_lower = entry.filename.to_lowercase();
        // strip frontmatter (YAML --- ... --- 块), 但保留 #tag 字面量供后续 tag 抽取用
        let body_lower = strip_frontmatter(full_md).to_lowercase();

        let idx_entry = MemoIndexEntry {
...
/// 简单 frontmatter stripper — 与 `memo_file.rs::extract_body_content` 行为一致,
/// 不引 `regex` 直接在 search.rs 内部处理 (输入已经在 `read_all_memos_with_body`
/// 阶段从 .md 文件读出来了, 不会有特殊性能压力).
...
fn make_snippet(entry: &MemoIndexEntry, query_lower: &str, field: &MatchField) -> String {
    ...
    let suffix = if end < source_chars.len() { ELLIPSIS } else { "" };
...
mod tests {
    ...
    fn mk_entry(
        ...
        let entry = MemoListEntry {
            ...
            favorited: false,
            icon: None,
            path: Some(format!("{}-{}.md", filename, id)),
        };
        let full_md = format!("---\nfilename: {}\n---\n{}", filename, body);
        (entry, full_md)
    }
    ...
    fn fixture_index() -> MemoIndex {
        ...
        let items = vec![
            mk_entry("m_001", "项目周报", "今天天气很好,适合写代码 #review", vec!["review"], 1000),
            mk_entry(
                "m_002",
                "Meeting notes Q2",
    ...
    fn search_bigram_false_positive_rejected() {
        ...
        idx.rebuild("nb".to_string(), vec![mk_entry("m_x", "x", "abc def", vec![], 0)]);
    ...
    fn snippet_is_utf8_safe() {
        ...
        idx.rebuild(
            "nb".to_string(),
            vec![mk_entry("m_zh", "中文", "这是一段非常非常非常非常长的中文内容,用来测试 snippet 在字符边界附近切片", vec![], 0)],
        );
