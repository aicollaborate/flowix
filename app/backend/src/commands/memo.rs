...
use crate::memo_file::{Memo, MemoFile, TodoItem};
...
/// 从 `{title}-{m_xxxxx}.md` 文件名解析 memo id. 解析不到 (非 memo 文件 /
...
pub fn write_document(
    ...
    match fs::write(&file_path, &content) {
        Ok(_) => {
            ...
            // (格式 `{title}-{m_xxxxx}.md`), 同步增量更新索引. 写入 `expectedContent`
            // 校验失败或写盘错误都走不到这里.
...
pub fn add_document(
    ...
    // 回退, 父目录一定存在, 这一步必然成功。
    let abs = state.memo_file.read().unwrap().get_memo_base().join(&memo_filename);
...
pub fn import_external_document_to_memo(
    ...
    let now = chrono::Utc::now().timestamp_millis();
    let id = generate_memo_id();
    let predicted_filename = format!("{}-{}.md", title, id);
...
pub fn update_memo_db(
    ...
    let predicted_filename = if let Some(ref f) = filename {
        format!("{}-{}.md", f, id)
    } else {
        // filename 不变 — 读 list.json 拿当前 filename
        state
            .memo_file
            .read()
            .unwrap()
