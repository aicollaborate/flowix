...
impl OpenAICompatibleProvider {
    ...
    ) -> Result<Pin<Box<dyn Stream<Item = Result<OpenAICompatibleStreamItem, RllmError>> + Send>>, RllmError>
    {
                        ...
                        for choice in response.choices {
                            ...
                            if let Some(reasoning) = delta.reasoning_content {
                                if !reasoning.is_empty() {
                                    ...
                                    queue.push_back(Ok(OpenAICompatibleStreamItem::Reasoning(reasoning)));
                                }
                            }
...
mod tests {
    ...
    fn parallel_tool_calls_get_their_own_buckets() {
        ...
        assert_eq!(calls.len(), 2, "expected 2 parallel tool calls, got {:?}", calls);
    ...
    fn single_tool_call_still_works_when_index_omitted() {
        ...
        // a known bucket.
        let mut pending = PendingToolCalls::new();
        let mut call = tc(0, "call_X", "list_notebooks", "{}");
    ...
    fn half_formed_buckets_are_skipped_at_flush() {
        ...
        let calls = flush_pending_tool_calls(&mut pending);
        assert!(calls.is_empty(), "half-formed bucket must be skipped, got {:?}", calls);
    }
