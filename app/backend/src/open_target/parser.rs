...
/// "用户想打开什么"。 resolver 层再查磁盘 / list.json 落到具体 notebook。
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase", tag = "kind", rename_all_fields = "snake_case")]
pub enum OpenTarget {
    /// 物理路径 — 走 list.json 扫所有 notebook 找匹配的 .md。
    PhysicalPath { path: String, memo_id: Option<String> },
...
/// 从文件名末尾抽 memo id (跟 `extract_memo_id_from_abs_path` 同形)。
/// 解析不到返回 None (非 memo 文件)。
pub fn extract_memo_id_from_filename(path: &str) -> Option<String> {
...
fn split_scheme<'a>(raw: &'a str) -> Option<(&'a str, &'a str)> {
    ...
    //   - rest **保留**原大小写 ── memo id 在 list.json 里走 `[0-9a-z]`,
...
pub fn parse_open_target(raw: &str) -> Result<OpenTarget, OpenTargetError> {
        ...
        .or_else(|| trimmed.strip_prefix("file:///"))
    {
        ...
        let memo_id = extract_memo_id_from_filename(&decoded);
    ...
    Ok(OpenTarget::PhysicalPath {
        ...
        memo_id: extract_memo_id_from_filename(trimmed),
    })
}
    ...
    match segments.as_slice() {
        ...
        ["open"] => {
            let path_arg = get_query(&query, "path")
                ...
                .to_string();
            let memo_id = extract_memo_id_from_filename(&path_arg);
            Ok(OpenTarget::DeepLink {
                url: full.to_string(),
                memo_id,
...
mod tests {
    ...
    fn parses_deep_link_memo_with_id() {
        ...
        match t {
            OpenTarget::DeepLink { memo_id, notebook_hint, physical_path, .. } => {
    ...
    fn parses_deep_link_memo_with_notebook_hint() {
        ...
        match t {
            OpenTarget::DeepLink { memo_id, notebook_hint, .. } => {
                assert_eq!(memo_id.as_deref(), Some("abc123"));
    ...
    fn parses_open_with_path_query() {
        let t = parse_open_target(
            "flowix://open?path=%2FUsers%2Frop%2FDocuments%2Fflowix%2Fnotebook%2Fhello%23vex4v9.md",
        )
        .unwrap();
        match t {
            OpenTarget::DeepLink { memo_id, physical_path, .. } => {
                assert_eq!(memo_id.as_deref(), Some("vex4v9"));
                assert_eq!(
                    physical_path.as_deref(),
                    Some("/Users/rop/Documents/flowix/notebook/hello#vex4v9.md")
                );
            }
            _ => panic!("expected DeepLink"),
        }
    ...
    fn parses_file_scheme() {
        let t =
            parse_open_target("file:///Users/rop/Documents/flowix/nb/hello%23vex4v9.md").unwrap();
        match t {
            OpenTarget::PhysicalPath { path, memo_id } => {
                assert_eq!(path, "/Users/rop/Documents/flowix/nb/hello#vex4v9.md");
                assert_eq!(memo_id.as_deref(), Some("vex4v9"));
            }
    ...
    fn parses_raw_absolute_path() {
        let t = parse_open_target("/Users/rop/Documents/flowix/nb/hello#vex4v9.md").unwrap();
        match t {
            OpenTarget::PhysicalPath { path, memo_id } => {
                assert_eq!(path, "/Users/rop/Documents/flowix/nb/hello#vex4v9.md");
                assert_eq!(memo_id.as_deref(), Some("vex4v9"));
    ...
    fn memo_id_with_unicode_path() {
        ...
        let t = parse_open_target("/Users/rop/Documents/flowix/开发待办事项/笔记#vex4v9.md").unwrap();
        match t {
            OpenTarget::PhysicalPath { path, memo_id } => {
                assert_eq!(path, "/Users/rop/Documents/flowix/开发待办事项/笔记#vex4v9.md");
                assert_eq!(memo_id.as_deref(), Some("vex4v9"));
            }
            _ => panic!("expected PhysicalPath"),
        }
