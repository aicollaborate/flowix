        ...
        match event.kind {
            ...
            | EventKind::Modify(_) => {
                ...
                if let Some(id) = MemoFile::extract_memo_id_from_abs_path(&path) {
                    // 5b. 文件名匹配 `{title}-{m_xxxxxx}.md` 约定 — 已有/历史 memo
...
fn unregister_and_emit(
    ...
    let Ok(mf) = memo_file.read() else { return; };
...
mod tests {
    ...
    #[test]
    fn extract_memo_id_parses_title_dash_id() {
        assert_eq!(
            MemoFile::extract_memo_id_from_abs_path(Path::new("/n/My Note-m_abc123.md")),
            Some("m_abc123".to_string())
        );
    }
    ...
    fn extract_memo_id_handles_unicode_title() {
        // 标题里含 `-` 是合法的 (Unicode U+002D 也是分隔符), rsplit 取最后一段
        assert_eq!(
            MemoFile::extract_memo_id_from_abs_path(Path::new("/n/a-b-c-m_x9.md")),
            Some("m_x9".to_string())
        );
    }
