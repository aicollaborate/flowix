...
impl MemoFile {
    ...
    pub(crate) fn get_notebook_file_path(&self) -> PathBuf {
    ...
    /// 无需建索引。
    pub(crate) fn get_notebook_config_by_id(&self, id: &str) -> Option<NotebookConfig> {
            ...
            if let Some(nb) = configs.iter().find(|n| n.is_default).cloned() {
                ...
                if !p.exists() {
                    ...
                    for cfg in configs.iter_mut() {
                        if cfg.is_default {
                            cfg.path =
                                format!("{}/", canonical_default.to_string_lossy());
                            cfg.updated_at = now;
                        }
                    }
