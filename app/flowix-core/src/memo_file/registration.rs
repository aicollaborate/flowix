...
//!   命名规范 (`{title}#xxxxxx.md`) 文件 → 复用 id, 不重命名; 任意命名 → 生成
//!   id + 重命名为规范名 (失败回滚)。
...
/// 跟 `flowix-desktop::fs_watcher::normalize_for_compare` 同口径:
/// `dunce::canonicalize` 失败时退到 "父目录 canonicalize + join 文件名",
/// 再失败退到 `path.to_path_buf()`。两份路径走这个函数归一后相等
/// → 视为同一文件 (兼容 macOS /private/var ↔ /var symlink 折叠)。
...
impl MemoFile {
    ...
    fn generate_memo_id(&self) -> String {
        loop {
    ...
    pub fn generate_memo_filename(title: &str, memoid: &str) -> String {
        let sanitized_title = Self::sanitize_memo_filename_component(title);
        let base = if sanitized_title.is_empty() {
            chrono::Local::now().format("untitled-%Y-%m-%d").to_string()
        } else {
            sanitized_title
        };

        format!("{}#{}.md", base, memoid)
    }

    pub fn memo_title_from_filename(name: &str) -> Option<String> {
    ...
    /// 从绝对路径 `/.../{title}#xxxxxx.md` 抽出 memo id, 用于 watcher
    ...
    /// 适用场景: Agent `write` 工具新建的 `{title}#xxxxxx.md` 文件;
    /// 外部工具在 notebook 目录里按命名约定新建 .md 后 watcher 观察到。
    ///
    /// 失败: 路径非 `#xxxxxx` 后缀 (不视为合法 memo 文件); 其它 IO 错。
    ...
    pub fn register_existing_file(&self, abs_path: &Path) -> Result<Memo, String> {
        let Some(id) = Self::extract_memo_id_from_abs_path(abs_path) else {
            return Err("path is not a named memo file (no `#xxxxxx` suffix)".to_string());
        };
        if !abs_path.exists() {
        ...
        let filename = abs_path
            .file_name()
            .and_then(|n| n.to_str())
            .and_then(Self::memo_title_from_filename)
            .unwrap_or_default();
        let mut memo = Memo {
            id: id.clone(),
            filename,
        ...
        memo.path = Some(Self::generate_memo_filename(&memo.filename, &id));
        MemoFile::sync_list_json_on_write(self, &memo)
            .map_err(|e| format!("sync list.json failed: {e}"))?;
        Ok(memo)
    }

    /// 把一个**任意命名**的 .md 文件注册为 memo, 并按 `{title}#xxxxxx.md`
    /// 规范重命名磁盘文件。适用: 外部工具 / 用户在 notebook 目录里手动创建一个
    /// 无 `#xxxxxx` 后缀的 .md (例如 `Agent 记忆管理.md`), watcher 抽不出
    /// id, 走本路径接管。
    ///
    /// 三件事:
    /// 1. 生成新 memo id (`xxxxxx`)。
    /// 2. **重命名磁盘文件**为 `{title}#xxxxxx.md` — 跟 `update_memo_item`
    ...
    pub fn register_unnamed_file(&self, abs_path: &Path) -> Result<(Memo, PathBuf), String> {
        ...
        // 跟 generate_unique_filename 同形: `{title}#xxxxxx.md`
        let new_filename = Self::generate_memo_filename(&title, &id);
        ...
        let final_memo = self.read_memo(&id).unwrap_or(memo);
    ...
    ///   一次性规范化。
    /// - 已经在 list.json 里的 .md 跳过 (按 id 去重)。
    /// - 命名规范 `{title}#xxxxxx.md` → 复用文件名里的 id。
    /// - 任意命名 .md → 生成新 id 后重命名为规范文件名。
    ...
    pub fn reconcile_with_disk(&self) -> Result<usize, String> {
        ...
        let mut existing_ids: std::collections::HashSet<String> =
            list.memos.iter().map(|e| e.id.clone()).collect();
        ...
        for entry in entries.filter_map(|e| e.ok()) {
            ...
            // 决定 id: 文件名带 #xxxxxx 后缀就复用, 否则生成新
            let file_name = match path.file_name().and_then(|n| n.to_str()) {
                Some(name) => name,
                None => continue,
            };
            let parsed_id = Self::extract_memo_id_from_abs_path(&path);
            if parsed_id
                .as_ref()
            ...
            let id = parsed_id.unwrap_or_else(|| self.generate_memo_id());
            let now = chrono::Utc::now().timestamp_millis();
            let mut memo = Memo {
                id: id.clone(),
                filename: Self::memo_title_from_filename(file_name).unwrap_or_default(),
                preview: String::new(),
                tags: vec![],
                todos: vec![],
                created_at: now,
            ...
            apply_derived_memo_fields(&mut memo, &content);
            let canonical_filename = Self::generate_memo_filename(&memo.filename, &id);
            if file_name != canonical_filename {
                let new_path = base.join(&canonical_filename);
                if fs::rename(&path, &new_path).is_err() {
                    continue;
                }
            }
            memo.path = Some(canonical_filename);
        ...
        for entry in &list.memos {
            ...
            let _ = self.sync_memo_json_todos_on_write(&Memo {
                ...
                path: Some(MemoFile::generate_memo_filename(&entry.filename, &entry.id)),
            });
        }
        Ok(added)
    }
    ...
    pub fn reload_memo_from_disk(&self, id: &str) -> Result<Memo, String> {
        ...
        apply_derived_memo_fields(&mut memo, &content);
        let canonical_filename = Self::generate_memo_filename(&memo.filename, id);
        if path.file_name().and_then(|n| n.to_str()) != Some(canonical_filename.as_str()) {
            fs::rename(&path, self.get_memo_base().join(&canonical_filename))
                .map_err(|e| format!("rename reloaded memo failed: {e}"))?;
        }
        memo.path = Some(canonical_filename);
    ...
    /// 或 entry 的 path 字段已经指向别的文件 (rename 旧文件 Remove 事件漏防)。
    ///
    /// ## Invariant guard (修复 list.json "item 被删除" 路径 2)
    ///
    /// 原实现只看 "list.json 中有 id 就删" ── 这无法区分:
    /// - A) 外部 `rm` 了 .md, list.json 该 id 仍残留 (需删)
    /// - B) 后端 `fs::rename(old, new)`, watcher 收到 old 的 Remove 事件,
    ///   list.json 已被 `sync_to_list_json_only` **先**写成新 path 字段
    ///   (修 fix 3 后), 但 Remove 事件漏过 mark_self_write 抑制 (normalize
    ///   失败 / 250ms id dedup 过期 / OS 时序错位)
    ///
    /// 区分方法: list.json entry 的 `path` 字段拼成绝对路径, 跟 abs_path
    /// 规范化后**相等**才删。rename 旧文件 → list.json 的 path 已经指向
    /// 新文件 → 不等 → 拒绝删 + 走 tracing::debug 记录。配合 fix 1 (list
    /// io 锁) + fix 3 (list 先于 rename 写), 这条防线能 100% 拦下 B 场景。
    pub fn unregister_memo_by_path(&self, abs_path: &Path) -> bool {
        let Some(id) = Self::extract_memo_id_from_abs_path(abs_path) else {
            return false;
        };
        let Some(entry) = self.read_memo(&id) else {
            return false;
        };

        // ── 新增 invariant guard ──
        // entry.path 是 list.json 里的**相对**文件名 (e.g. "Title#abc123.md"),
        // 拼成绝对路径后跟 abs_path 规范化比对。
        let Some(rel_filename) = entry.path.as_deref() else {
            // list.json 中 path 字段缺失 ── `find_memo_file_by_id` 会用
            // filename + id 拼一个, 但我们这里没那一步, 直接放过: 旧实现
            // 也会删, 这里保守保留旧行为 (entry.path 为 None 多见于老数据)。
            return MemoFile::sync_list_json_on_delete(self, &id).is_ok();
        };
        let base = self.get_memo_base();
        let expected_abs = base.join(rel_filename);
        // 用 `dunce::canonicalize` 跟 watcher 端 `normalize_for_compare` 对齐
        // 口径 (macOS /private/var ↔ /var symlink 折叠), 但 `dunce` 在
        // flowix-core 依赖里可能没引, 这里退到 "父目录 canonicalize + join
        // 文件名" 的 watch 端同形 fallback。
        // 跟 `fs_watcher::normalize_for_compare` 同口径 (`dunce::canonicalize` +
        // 父目录 fallback + `path.to_path_buf()` 兜底)。两份归一形式相等
        // 才认为 "list.json path 字段真的指向 abs_path"。
        let expected_norm = normalize_for_compare(&expected_abs);
        let observed_norm = normalize_for_compare(abs_path);
        if expected_norm != observed_norm {
            // list.json 的 path 字段不指向这个 abs_path ── 拒绝删
            tracing::debug!(
                "[unregister_memo_by_path] refused: id={id} list.json.path={} but got abs_path={}                  (likely rename 旧文件 Remove 事件漏防, 已被 invariant guard 拦下)",
...
mod tests {
    ...
    fn generate_memo_id_random_segment_is_lowercase_alphanumeric() {
        let mf = MemoFile::default();
        for _ in 0..50 {
            let id = mf.generate_memo_id();
            assert_eq!(id.len(), 6, "随机段必须 6 位, got {id}");
            assert!(
                id.chars()
                    .all(|c| c.is_ascii_digit() || c.is_ascii_lowercase()),
                "随机段含非小写字母+数字字符: {id}"
