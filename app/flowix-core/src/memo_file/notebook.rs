...
impl MemoFile {
    ...
    /// Registered notebook directories. The default directory is always
    /// included so a fresh profile still has a valid notebook root.
    ...
    pub fn init_default_notebook(&self) -> NotebookConfig {
        ...
        let mut configs = match self.read_notebook_configs() {
            ...
            Err(e) => {
                tracing::error!(
                    "failed to read notebook registry {}; default notebook not persisted: {e}",
                    self.get_index_db_path().display()
                );
                return default_nb;
            }
        };
