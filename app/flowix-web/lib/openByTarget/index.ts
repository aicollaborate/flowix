 ...
 *   - 主动入口 (`openNoteByDeepLink` / `openNoteByPhysicalPath`) 供
 *     noteReference 双击、Agent 工具调用、跨窗口等热路径用, 同步走完整
 *     IPC 解析, 不依赖事件订阅。
 *
 * 用法:
 *   - App.tsx 顶层:
 *       useEffect(() => { void mountOpenTargetListener(); return unmount; }, []);
 *   - noteReference 双击 / markdown-paste:
 *       await openNoteByPhysicalPath(rawPath);
 */
