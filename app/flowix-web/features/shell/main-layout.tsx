...
export function MainLayout() {
  ...
  const rightPanelVisible = agentPanelVisible;
  // tags 面板独立成最左列, 宽度走自己的 state; 与 agent 面板互不影响。
  ...
  const handlePanelSwipe = useCallback((direction: MacosTrackpadSwipeDirection) => {
    const transition = resolvePanelSwipeTransition(
      { memoListVisible, noteNavigationVisible, agentPanelVisible },
      direction,
    );
      ...
      if (ok) {
        ...
        if (nbList) {
          ...
          if (selectedNotebook?.id === target.id) {
            const defaultNotebook = nbList.find((notebook) => notebook.isDefault) ?? nbList[0] ?? null;
            setSelectedNotebook(defaultNotebook);
            setSelectedMemo(null);
            clearDocument();
            await notebooksClient.setCurrent(defaultNotebook?.id ?? null);
            triggerRefresh();
          }
        }
      } else {
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          ...
          {/* Agent chat panel divider */}
            <div
              className={`relative h-full group z-10 overflow-hidden ${
                ...
                rightPanelVisible ? 'cursor-col-resize opacity-100' : 'pointer-events-none opacity-0'
              }`}
              style={{ width: rightPanelVisible ? 1 : 0, flexShrink: 0 }}
              onMouseDown={rightPanelVisible ? handleAgentDividerMouseDown : undefined}
            >
              <div className="absolute inset-0 -translate-x-1/2 w-[12px] left-1/2 bg-transparent z-11" />
          ...
          {/* Agent chat panel */}
            ...
            >
              ...
              >
                {agentPanelVisible && <LazyAgentPanel onClosePanel={handleToggleAgentPanel} />}
              </div>
            </div>
          </div>
          {/* Status bar */}
