...
function getAdjacentBlockImageRange($pos: any, direction: 'before' | 'after'): { from: number; to: number } | null {
    if ($pos.depth <= 0) return null;

    const textblockDepth = $pos.depth;
    const parentDepth = textblockDepth - 1;
    const parent = $pos.node(parentDepth);
    const currentBlock = $pos.node(textblockDepth);

    if (direction === 'before') {
        if ($pos.parentOffset !== 0) return null;
        const currentIndex = $pos.index(parentDepth);
        if (currentIndex <= 0) return null;
        const previousNode = parent.child(currentIndex - 1);
        if (previousNode.type.name !== 'image') return null;
        const currentBlockStart = $pos.before(textblockDepth);
        return { from: currentBlockStart - previousNode.nodeSize, to: currentBlockStart };
    }

    if ($pos.parentOffset !== currentBlock.content.size) return null;
    const nextIndex = $pos.indexAfter(parentDepth);
    if (nextIndex >= parent.childCount) return null;
    const nextNode = parent.child(nextIndex);
    if (nextNode.type.name !== 'image') return null;
    const currentBlockEnd = $pos.after(textblockDepth);
    return { from: currentBlockEnd, to: currentBlockEnd + nextNode.nodeSize };
}
    ...
    addKeyboardShortcuts() {
        return {
            Backspace: () => {
                ...
                const range = getAdjacentBlockImageRange(selection.$from, 'before');
            ...
            Delete: () => {
                ...
                const range = getAdjacentBlockImageRange(selection.$from, 'after');
                if (!range) return false;
                this.editor.commands.deleteRange(range);
                return true;
            },
