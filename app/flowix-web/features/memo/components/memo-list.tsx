...
import { listenToNotebookImportComplete } from '@platform/tauri/client';
...
export function MemoList() {
  ...
  const normalizedVisibleMemoCount = Math.min(memos.length, Math.max(visibleMemoCount, minimumVisibleMemoCount));
  const renderedMemos = useMemo(
    () => memos.slice(0, normalizedVisibleMemoCount),
    [memos, normalizedVisibleMemoCount]
  );
  const hasMoreMemos = normalizedVisibleMemoCount < memos.length;
  ...
  const loadMoreRenderedMemos = useCallback(() => {
    setVisibleMemoCount((count) => {
      ...
      return Math.min(memos.length, nextCount);
    });
