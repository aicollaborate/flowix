...
function buildCurrentNoteBlock(content: string): string {
  const trimmed = content.trim();
  return trimmed ? `<current-note>\n${trimmed}\n</current-note>` : '';
}
      ...
      return {
        ...
        sendMessageToThread: async (threadId, content, roleKey, options) => {
          ...
          const currentNoteBlock =
            currentMessages.length === 0 && options?.currentNoteContent
              ? buildCurrentNoteBlock(options.currentNoteContent)
              : '';
          const llmContent = currentNoteBlock
