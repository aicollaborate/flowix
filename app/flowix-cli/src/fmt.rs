...
/// `NOTES` 来自 `list.json` 的 entry 数; 暂不递归 (跟 desktop 端一致)。
pub fn print_notebooks(configs: &[NotebookConfig]) {
    ...
    // 收集笔记数 ── 读每个 notebook 的 `.metadata/list.json` 拿 entry 数。
    // M1 简化: 没找到 list.json 就显示 `-`。
    let rows: Vec<Row> = configs
        .iter()
        .map(|c| {
            let notes = count_list_entries(&c.path);
            Row {
                name: c.name.clone(),
                id: c.id.clone(),
...
/// 读 `<notebook>/.metadata/list.json`, 返回 entry 数。
/// 读不到 (新建空 notebook / 还没建索引) 返回 `-` 显示。
fn count_list_entries(notebook_path: &str) -> usize {
...
use flowix_core::memo_file::MemoListEntry;

/// 打印某 notebook 下的笔记列表 (来自 list.json 的 memos 数组)。
///
/// 表格列: ID  TITLE  TAGS  FAV  UPDATED
/// - TITLE 截断到 32 字符 (CJK 算 2)
/// - TAGS 显示逗号分隔, 截断到 16 字符
/// - FAV 布尔显示 ★ / -
pub fn print_notes(entries: &[MemoListEntry]) {
    if entries.is_empty() {
        println!("(no notes)");
        return;
    }
...
pub fn print_note(entry: &MemoListEntry, body: &str) {
    println!("---");
    println!("id:        {}", entry.id);
...
/// 打印 notebooks JSON. 顶层是数组, 每项含 name / id / path / notes / updated。
    ...
    let arr: Vec<serde_json::Value> = configs
        ...
        .map(|c| {
            serde_json::json!({
                ...
                "notes": count_list_entries(&c.path),
...
/// 打印 notes JSON. 顶层数组, 每项是 MemoListEntry 完整字段。
pub fn print_notes_json(entries: &[flowix_core::memo_file::MemoListEntry]) {
    let json = serde_json::to_string_pretty(entries).unwrap_or_default();
    println!("{json}");
}

/// 打印单条 note JSON. 顶层对象, 包含 metadata + body。
pub fn print_note_json(entry: &flowix_core::memo_file::MemoListEntry, body: &str) {
