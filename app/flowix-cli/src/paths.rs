...
/// **且** 新目录不存在 (避免覆盖)。
///
/// 任何步骤出错都 `eprintln!` 但不中断启动 ── 用户数据在原位仍然
/// 可读。**此操作不可逆**: 旧目录在 copy 成功后被 `remove_dir_all` 删除。
        ...
        match copy_dir_recursive(&old_cfg, &new_cfg) {
            Ok(()) => {
                if let Err(e) = std::fs::remove_dir_all(&old_cfg) {
                    eprintln!("[flowix-cli] warn: failed to remove legacy ~/.woop after copy: {e}");
                } else {
                    eprintln!("[flowix-cli] info: migrated ~/.woop → ~/.flowix");
                }
            }
            ...
            match copy_dir_recursive(&old_data, app_data_path) {
                Ok(()) => {
                    if let Err(e) = std::fs::remove_dir_all(&old_data) {
                        eprintln!("[flowix-cli] warn: failed to remove legacy app data dir: {e}");
                    } else {
                        eprintln!("[flowix-cli] info: migrated app data: woopmemo → flowix");
                    }
                }
                Err(e) => eprintln!(
                    "[flowix-cli] warn: failed to copy app data: woopmemo → flowix: {e}"
                ),
            }
        }
    }
